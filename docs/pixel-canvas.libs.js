class DirectedSegments {
    directedSegments = [];
    addPixel(x, y) {
        this.directedSegments.push({ sx: x, sy: y, ex: x + 1, ey: y }, { sx: x + 1, sy: y, ex: x + 1, ey: y + 1 }, { sx: x + 1, sy: y + 1, ex: x, ey: y + 1 }, { sx: x, sy: y + 1, ex: x, ey: y });
    }
    notDuplicate() {
    ...
    annihilate() {
        const directedSegments = [...this.directedSegments];
        ...
        return directedSegments.filter((directedSegment) => {
            return directedSegment !== null;
        });
    }
    findCorner(directedSegments, start) {
        let min = start;
        for (let i = start + 1; i < directedSegments.length; ++i) {
            if (directedSegments[i].ex <= directedSegments[i].sx) {
                continue;
            }
            if (directedSegments[i].sy < directedSegments[min].sy ||
    ...
    connect(directedSegments, start) {
        for (let i = start + 1; i < directedSegments.length; ++i) {
    ...
    directedSegmentToVector(directedSegment) {
    ...
    merge(directedSegments) {
        let start = null;
        let target = -1;
        return directedSegments.map((directedSegment, index) => {
            if (!start) {
    ...
    directedSegmentToString(directedSegment) {
